...
async def test_get_user(async_client: AsyncClient):
    ...
    get_other_user = await async_client.get(
        f'{base_url}/example3',
        headers=headers
    )
    ...
    get_not_exist_user = await async_client.get(
        f'{base_url}/undefined',
        headers=headers
    )
    assert get_not_exist_user.status_code == 404

    get_all_users = await async_client.get(
        f'{base_url}/all',
        headers=headers 
    )
...
async def test_get_user_rows(async_client: AsyncClient):
    ...
    get_user_chats = await async_client.get(
        f'{base_url}/{user}/chats',
        headers=headers
    )
    assert get_user_chats.status_code == 200

    get_user_posts = await async_client.get(
