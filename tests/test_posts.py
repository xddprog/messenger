...
async def test_get_posts(async_client: AsyncClient):
    ...
    get_all_posts = await async_client.get(
        f'{base_url}/all',
        headers=headers
    )
    assert get_all_posts.status_code == 200

    get_one_post = await async_client.get(
        f'{base_url}/{test_post}',
        headers=headers
    )
    assert get_one_post.status_code == 200

    delete_post = await async_client.delete(
        f'{base_url}/{test_post}',
        headers=headers
    )
